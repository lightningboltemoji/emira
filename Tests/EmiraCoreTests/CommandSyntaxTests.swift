...
@Suite struct CommandSyntaxTests {
    ...
    static let all: [Command] = [
        ...
        .setLayout(.strip), .setLayout(.stack),
        .fullscreen(.on), .fullscreen(.off), .fullscreen(.toggle),
        .float(.on), .float(.off), .float(.toggle),
        .pin(.left), .pin(.toggleLeft), .pin(.right), .pin(.toggleRight), .pin(.off),
        .focusPinned,
    ...
    @Test func everySignatureIsSpelledExactlyAsItWas() {
        let expected: [String: String] = [
            ...
            "pin": "pin <left|toggle-left|right|toggle-right|off>",
